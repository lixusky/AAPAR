...
from losses.cross_entropy_loss import *
...
ViT_model, ViT_preprocess = clip.load("ViT-L/14", device=device,download_root='/media/sdd/lx/pretrained/') 
...
def main(cfg, args):
    ...
    model = TransformerClassifier(args,train_set.attr_num,train_set.label_word, train_set.des_num)
    ...
    criterion = CEL_Sigmoid(label_ratio, attr_idx=train_set.attr_num)
    
    criterion_clip=CrossEntropyLoss(num_classes=train_set.des_num)
    
    clip_params=[]
    for name, param in ViT_model.named_parameters():
        if any(keyword in name for keyword in cfg.TRAIN.CLIP_PARAMS):#clip_update_parameters
            print(name, param.requires_grad)
            clip_params.append(param)
        else:
            param.requires_grad = False
    optimizer = optim.Adam([{'params':params} for params in clip_params]+[{'params':model.parameters()}], lr=cfg.TRAIN.LR_SCHEDULER.LR_FT, weight_decay=cfg.TRAIN.OPTIMIZER.WEIGHT_DECAY)
    # lr_scheduler = create_scheduler(optimizer, num_epochs=cfg.TRAIN.MAX_EPOCH, lr=cfg.TRAIN.LR_SCHEDULER.LR_FT, warmup_t=5)#5
    ...
    best_metric, epoch = trainer(cfg, args, epoch=cfg.TRAIN.MAX_EPOCH,
                                 model=model, ViT_model=ViT_model,model_ema=model_ema,
...
def trainer(cfg, args, epoch, model,ViT_model, model_ema, train_loader, valid_loader, criterion,criterion_c, optimizer, lr_scheduler,
            path, loss_w, viz, tb_writer,des_non):
    ...
    for e in range(1,epoch+1):
        ...
        if cfg.METRIC.TYPE == 'pedestrian':
            ...
            if cur_metric > maximum:
                maximum = cur_metric
                best_epoch = e
                save_ckpt(model,ViT_model, path, e, maximum)
