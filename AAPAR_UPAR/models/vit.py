...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.attn_wt=torch.tensor(0).cuda()
    ...
    def forward(self, x,attn_mask=None):
        ...
        self.attn_wt=attn.sum(dim=1) / self.num_heads
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU,attn_mask: torch.Tensor = None, norm_layer=nn.LayerNorm):
        super().__init__()
    ...
    def forward(self, x):#这里加一下
        if self.attn_mask==None:
            ...
            attn=self.attn(self.norm1(x),attn_mask=self.attn_mask)
        self.attn_w=self.attn.attn_wt
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.blocks = nn.ModuleList([
            Block(
                ...
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[i], attn_mask=None,norm_layer=norm_layer)
